...
var vm = Vue.createApp({
    ...
    created: function created() {
        var self = this;
        self.signalRconnect = new signalR.HubConnectionBuilder().withUrl(self.config.ChatHubUrl) // 你的 SignalR Hub 地址
        .withAutomaticReconnect().build();
        self.initSigmalR(self);
    },
    methods: {
        initSigmalR: function initSigmalR() {
            var self = arguments.length <= 0 || arguments[0] === undefined ? this : arguments[0];
            return regeneratorRuntime.async(function initSigmalR$(context$1$0) {
                while (1) switch (context$1$0.prev = context$1$0.next) {
                    case 0:
                        context$1$0.next = 2;
                        return regeneratorRuntime.awrap(self.signalRconnect.start().then(function () {
                    ...
                    case 2:
                        ...
                        self.signalRconnect.on("UserConnected", function (onlineuser) {
                            //將新加入的使用者新增到聊天對象清單
                            if (onlineuser.connectionID != self.connectionid) self.chatlist.push(onlineuser);
                        });
                        //監聽使用者離線
                        self.signalRconnect.on("UserDisconnected", function (offlineuserid) {
                            var nowusers = self.chatlist;
                            //確認聊天對象清單是否存在該使用者
                            var user = _.find(nowusers, function (o) {
                                return o.connectionID == offlineuserid;
                            });

                            //如果有的話就取得不包含該離線者的資料重新放入聊天對象清單
                            if (user != null && user !== undefined) self.chatlist = _.filter(nowusers, function (o) {
                                return o.connectionID != user.connectionID;
                            });
                        ...
                        self.signalRconnect.on("PrivateMessage", function (senduser, message) {
                            ...
                            //抓取目前清單裡有沒有這個人
                            var user = _.find(nowusers, function (o) {
                                return o.connectionID == senduser.connectionID;
                            });
                            ...
                            if (user == null || user == undefined) {
                                ...
                                if (self.nowtalkid != user.connectionID) user.noReadCount = user.noReadCount + 1;
        ...
        talkselect: function talkselect(selectid) {
            ...
            var nowusers = this.chatlist;
            var user = _.find(nowusers, function (o) {
                return o.connectionID == selectid;
            });
            //判斷使用者清單沒有這個人就加上
            if (user != null && user != undefined) {
        ...
        sendmessage: function sendmessage() {
            var nowusers = this.chatlist;
            var nowtalk = this.nowtalkid;
            if (this.isWhiteSpace(this.keyonmessage)) return;
        ...
        addTalk: function addTalk(talkid, sayid, message) {
            //將收到的訊息加到對話清單裡
            var talk = _.find(this.talklist, function (o) {
                return o.talkid == talkid;
            });
            ...
            if (talk.talkid == this.nowtalkid) this.refreshChat();
        },
        //刷新當前對話內容
        refreshChat: function refreshChat() {
            ...
            var nowtalkid = this.nowtalkid;
            var nowtalks = _.find(this.talklist, function (o) {
                return o.talkid == nowtalkid;
            }); //o.talkid.includes(nowtalkid)
        ...
        autoScrollToBottom: function autoScrollToBottom() {
            // 抓取scrollbar的區塊
            var container = this.$refs.scrollContainer;
            if (container.scrollHeight == undefined) return;
            container.scrollTop = container.scrollHeight;
        },
        //判斷是否有空白
        isWhiteSpace: function isWhiteSpace(text) {
            var regex = /^\s*$/;
